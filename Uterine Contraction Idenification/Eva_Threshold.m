...
Training_Czech_G2 = readmatrix('Czech Y_Y Y_M M_Y.csv');
Training_Czech_G3 = readmatrix('Czech Y_Y Y_M M_Y M_M.csv');

Training_SBU_G1 = readmatrix('SBU Y_Y.csv');
Training_SBU_G2 = readmatrix('SBU Y_Y Y_M M_Y.csv');
Training_SBU_G3 = readmatrix('SBU Y_Y Y_M M_Y M_M.csv');
...
%         end
        ...
        if ~isempty(locs) && (pointer<locs(end)) && (n ~= numel(t))
              [~,candidates] = contraction_detector_Threshold(Detua(pointer+1:locs(end)),[Amin, Tmin, DL],pointer);
        ...
        elseif n == numel(t)
            [~,candidates] = contraction_detector_Threshold(Detua(pointer+1:receivedLen),[Amin, Tmin, DL],pointer);
...
%     % ======================================    
    ...
    Eva1_ThresholdDetector{i}.ID = id;
    Eva1_ThresholdDetector{i}.Contractions = ContractionsSaving;
    Eva1_ThresholdDetector{i}.GroundTruth = [onsetTrue' offsetTrue'];
    K = length(onsetTrue);
    IOU = zeros(1,K);
    for k=1:K
    ...
    Eva1_ThresholdDetector{i}.IOU = IOU;

end

save Eva1_ThresholdDetector_Czech Eva1_ThresholdDetector
...
for i = 1:15
    switch Czechlabel(i)
        case 'g'
            IOUg = [IOUg Eva1_ThresholdDetector{i}.IOU];
        case 'a'
            IOUa = [IOUa Eva1_ThresholdDetector{i}.IOU];
        case 'm'
            IOUm = [IOUm Eva1_ThresholdDetector{i}.IOU];
        case 'b'
            IOUb = [IOUb Eva1_ThresholdDetector{i}.IOU];
        case 'p'
            IOUp = [IOUp Eva1_ThresholdDetector{i}.IOU];
    end
end
% Eva1_OnlyEnergyDetector_Czech = [mean(IOUg) mean(IOUa) mean(IOUm) mean(IOUb) mean(IOUp)];
Eva1_ThresholdDetector_Czech = [median(IOUg) median(IOUa) median(IOUm) median(IOUb) median(IOUp)];

plot(Eva1_ThresholdDetector_Czech,'*-');
...
for i = 1:15
    ...
    plot(SIGua);
    hold on;
    for k = 1:size(Eva1_ThresholdDetector{i}.GroundTruth,1)
        plot(Eva1_ThresholdDetector{i}.GroundTruth(k,1):Eva1_ThresholdDetector{i}.GroundTruth(k,2),95*ones(1,length(Eva1_ThresholdDetector{i}.GroundTruth(k,1):Eva1_ThresholdDetector{i}.GroundTruth(k,2))),'r-','LineWidth',6);
    end
    for k = 1:size(Eva1_ThresholdDetector{i}.Contractions,1)
        patch([Eva1_ThresholdDetector{i}.Contractions(k,1) Eva1_ThresholdDetector{i}.Contractions(k,2) Eva1_ThresholdDetector{i}.Contractions(k,2) Eva1_ThresholdDetector{i}.Contractions(k,1)], [150 150 0 0], 'y', 'FaceAlpha', .3);
    end
end
...
save Eva1_ThresholdDetector_SBU Eva1_ThresholdDetector
...
Eva1_ThresholdDetector_SBU = [median(IOUg) median(IOUa) median(IOUm) median(IOUb) median(IOUp)];

figure
plot(Eva1_ThresholdDetector_SBU,'*-');
